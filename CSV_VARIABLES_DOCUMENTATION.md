...
The task supports **fMRI/EEG synchronization** via a photodiode detector and parallel-port TTL triggers.
...
- **Description**: Time when the fixation cross first appeared (photodiode/TTL trigger moment) before this image
...
### `question_trigger`
- **Type**: Float (Unix timestamp) or None
- **Description**: Time when the object question first appeared (photodiode/TTL trigger moment)
