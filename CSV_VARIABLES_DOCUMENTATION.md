...
Photodiode (0.03 × 0.01): **Touch screen** at (-0.70, -0.48); **keyboard** at (-0.75, -0.48). **Present in both tasks** (main experiment and localizer). **White baseline** when nothing is happening. At each event (fixation onset/offset, image onset/offset, instruction onset, outcome onset, participant commits), the patch **flashes black** (TTL sent) **then white**. **Outcome onset** triggers photodiode flash, TTL, and CSV write in **both touch-screen and keyboard modes**.—a quick transition via `win.flip()`; it does not stay black. **Every flash is accompanied by a TTL trigger** via parallel port (Windows/Linux; default 0x0378; `PARALLEL_PORT_ADDRESS` to override).
...
### `outcome_trigger`
- **Type**: Float (Unix timestamp) or None
- **Description**: Time when the outcome screen (Correct/Incorrect) first appeared (photodiode/TTL trigger moment). Photodiode flashes, TTL fires, and event is logged to `recognition_ttl_events_*.csv` in both touch-screen and keyboard modes.
