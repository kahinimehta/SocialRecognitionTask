...
Photodiode (0.03 × 0.01, extreme right at 0.49, -0.45), white baseline. **Flashes black** on: fixation onset/offset, image onset/offset, instruction onset, outcome onset, participant commits (SUBMIT, CONTINUE, BEGIN, stay/switch). **TTL sent with every black flash** via parallel port (Windows/Linux; default 0x0378; `PARALLEL_PORT_ADDRESS` to override). Not shown until first task screen. Localizer: YES/NO questions do not trigger flashes.

### CSV Trigger Variables

All `*_trigger` variables in the CSVs record the **Unix timestamp** when the photodiode (and TTL) fired for that event. Use these to align behavioral data with neural recordings:

| Task | Variables | Event |
|------|-----------|-------|
| **Study** | `study_fixation_trigger` | Fixation onset (photodiode/TTL) |
| **Study** | `study_image_trigger` | Study image onset (photodiode/TTL) |
| **Recognition** | `recognition_fixation_trigger` | Fixation onset (photodiode/TTL) |
...
| **Localizer** | `localizer_fixation_trigger` | Fixation onset (photodiode/TTL) |
| **Localizer** | `localizer_image_trigger` | Image onset |
| **Localizer** | `question_trigger` | Question screen onset (behavioral; no photodiode on YES/NO) |

### When the Photodiode Is *Not* Shown

- Input method selection screen (`temp_win`)
- Participant name entry screen
...
**Columns (recognition_study)**: `block`, `phase`, `trial`, `image_path`, `study_fixation_trigger`, `fixation_offset`, `fixation_duration`, `study_image_trigger`, `image_offset`, `image_duration`
...
### `study_fixation_trigger`
- **Type**: Float (Unix timestamp)
- **Description**: Time when the fixation cross first appeared (photodiode/TTL fired) before this image
...
### `study_image_trigger`
- **Type**: Float (Unix timestamp)
- **Description**: Time when the study image first appeared (photodiode/TTL trigger moment)
...
**Columns (recognition_trials)** — photodiode/trigger variables: `recognition_fixation_trigger`, `recognition_image_trigger`, `switch_stay_trigger`, `outcome_trigger` (plus existing RTs, accuracy, etc.)
...
### `recognition_fixation_trigger`
- **Type**: Float (Unix timestamp)
- **Description**: Time when the pre-trial fixation first appeared (photodiode/TTL trigger moment)
- **Example**: `1764818191.994316`

### `recognition_image_trigger`
- **Type**: Float (Unix timestamp)
- **Description**: Time when the recognition image first appeared (photodiode/TTL trigger moment)
...
### `outcome_trigger`
- **Type**: Float (Unix timestamp) or None
- **Description**: Time when the outcome screen (Correct/Incorrect) first appeared (photodiode/TTL trigger moment)
...
- **Photodiode/TTL**: Every `*_trigger` = Unix timestamp when photodiode/TTL fired. Slider: fires on **submit only**, not on movement.
...
- **Type**: Float (Unix timestamp)
- **Description**: Time when the fixation cross first appeared before this image (photodiode/TTL trigger moment)
- **Note**: Fixation appears before EVERY image, including the first image
- **Example**: `1764818170.5`
...
### `localizer_image_trigger`
- **Type**: Float (Unix timestamp)
- **Description**: Time when the image first appeared (photodiode/TTL trigger moment)
- **Example**: `1764818171.2572181`

### `image_offset_time`
- **Type**: Float (Unix timestamp)
- **Description**: Time when the image was removed from display (in seconds since epoch, high precision)
- **Note**: Images are displayed for exactly 0.5 seconds, so `image_offset_time - localizer_image_trigger = 0.5`
...
### `question_trigger`
- **Type**: Float (Unix timestamp) or None
- **Description**: Time when the object question first appeared (behavioral timestamp; YES/NO screen does not trigger photodiode)
